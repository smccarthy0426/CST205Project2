...
def music_select_screen():        
    ...
    pygame.display.set_mode([pygame.Surface.get_width(pygame.image.load('maingamebackgroundimage.jpg').convert()),pygame.Surface.get_height(pygame.image.load('maingamebackgroundimage.jpg').convert())])
            
    #Music Select Screen
    font = pygame.font.SysFont("helvetica", 32)
    text = font.render("Please Wait...", 1, (255, 255, 255))
    textpos = text.get_rect()
    textpos.centerx = background.get_rect().centerx
    background.blit(text, textpos) 

    text3 = font.render("Use the up and down arrow keys to move Nyan Cat.", 1, (255, 255, 255))
    textpos3 = text.get_rect()
    textpos3.centery = background.get_rect().centery +0
    textpos3.centerx = background.get_rect().centerx - 250
    background.blit(text3, textpos3)
    font = pygame.font.SysFont("helvetica", 30)
    text4 = font.render("Try to avoid the blocks.", 1, (255, 255, 255))
    textpos4 = text.get_rect()
    textpos4.centery = background.get_rect().centery +25
    textpos4.centerx = background.get_rect().centerx - 250
...
def gameLoop():
                ...
                while not gameEnd:
                    ...
                    #Give the enemies a new starting position after the enemies pass the cat and add one point to score
                    if enemy1_x <= 0:
                        score += 1
                        enemy1_x = display_w - block_size
